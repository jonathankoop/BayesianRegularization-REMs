...
## Reading in the Data
```{r}
data <- read.csv("../../RawData/Spotify/Spotify_long_trimmed.csv")
data_raw <- read.csv("../../RawData/Spotify/Spotify_long.csv")
```
...
data %>%
  filter(grepl("german", genre, ignore.case = TRUE))
```
...
artist_popularity <- data_raw %>%
  dplyr::select(Artist_Names, Artist_Popularity) %>%
...
data_analysis <- data_wide %>%
  ...
  dplyr::select(release_date, Artist_1, Artist_2, genre)
...
artists <- data_analysis %>%
  dplyr::select(starts_with("Artist_")) %>%
...
# Start a RSelenium server and browser session (adjust 'chromever' according to your installed Chrome version)
rD <- rsDriver(browser = "chrome", chromever = "latest", port = 4545L)
remDr <- rD[["client"]]
...
```{r}
load("../../RawData/Spotify/gender_lastfm.RData")
```
...
# save props table
save(props, file = "Data/proportions-by-pop-thres.RData")
```
...
## Subset to Popular Artists

```{r}
popular_artists <- artist_popularity %>% 
  arrange(desc(Artist_Popularity)) %>% 
  head(100) %>%
  dplyr::select(Artist_Names) %>% 
  pull()

data_popular <- data_analysis %>% 
  filter(Artist_1 %in% popular_artists & Artist_2 %in% popular_artists) %>% 
  mutate(Gender_Artist_1 = gender_lastfm[Artist_1],
         Gender_Artist_2 = gender_lastfm[Artist_2]) %>% 
  filter(Gender_Artist_1 %in% c("male", "female") &
           Gender_Artist_2 %in% c("male", "female"))
```


```{r}
# subset to artists that collaborated at least twice
artists_freq <- c(data_popular$Artist_1, data_popular$Artist_2) %>%
  table() %>%
  as.data.frame(
    stringsAsFactors = FALSE
  ) %>%
...
data_popular <- data_popular %>% 
  filter(Artist_1 %in% artists_freq & Artist_2 %in% artists_freq)
```


```{r}
save(data_popular, file =  "../../CleanedData/Data_Popular_100.RData")
```

```{r}
covar <- artists_freq %>%
  as.data.frame() %>%
  dplyr::rename(name = !!sym(".")) %>% 
  mutate(
    gender = gender_lastfm[name],
    gender = recode(gender, `male` = 0, `female` = 1),
    popularity = artist_popularity$Artist_Popularity[match(name, artist_popularity$Artist_Names)],
    time = 0
  ) %>%
  filter(gender %in% c(0, 1))

save(covar, file =  "../../CleanedData/covar_spotify.RData")
```
## Analyze Genre Levels

```{r}
library(stringr)

# Step 1 & 2: Extract and clean genres from each string
genre_list <- lapply(data_raw$genre, function(x) {
  # Extract tokens between single quotes
  tokens <- str_extract_all(x, "(?<=')[^']+(?=')")[[1]]
  # Trim any whitespace
  tokens <- trimws(tokens)
  # Remove empty tokens
  tokens <- tokens[tokens != ""]
  # Filter to keep tokens that are likely valid genre names 
  # (allow letters, numbers, spaces, ampersands, hyphens, and apostrophes)
  tokens <- tokens[grepl("^[a-zA-Z0-9 &'-]+$", tokens)]
  unique(tokens)
})

# Step 3: Flatten the list and get frequency counts
all_genres <- unlist(genre_list)
genre_freq <- sort(table(all_genres), decreasing = TRUE)
top_genres <- names(genre_freq)[1:10]
print(top_genres)

data_raw$simple_genre <- sapply(genre_list, function(genres) {
  # Find intersection with the top genres
  common <- intersect(genres, top_genres)
  if (length(common) > 0) {
    # Optionally, pick the first one (or use your own criteria)
    return(common[1])
  } else {
    return("Other")
  }
